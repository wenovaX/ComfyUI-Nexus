...
Nexus for ComfyUI is a native desktop companion for ComfyUI.<br>
...
Use a short install path such as `D:\Nexus` or `C:\AI\Nexus`.<br>
Very deep folders, synced cloud folders, or protected locations can cause
Python package installs to hit Windows path-length or permission issues.
...
Nexus expects `App` and `LocalRuntime\Packages` beside it so Python, Git, the Nexus bridge,
and the verified ComfyUI source fallback can be prepared without an installer.
...
- Choose between the latest remote ComfyUI source and the verified Nexus-bundled
  ComfyUI source fallback when preparing a managed runtime.
- Validate Python, Git, ComfyUI, required packages, and virtual environments.
- Keep external model libraries aligned with ComfyUI's `extra_model_paths.yaml`
  before server boot.
...
- Verify shell services, server processes, and the local listener before a restart or server shutdown continues.
- Follow pooled live boot logs, installer progress, and server readiness without leaving the app.
...
- Use the managed HUD and independent Nexus bridge for tighter native/web coordination.
...
- Windows 10 or later
- WebView2 Runtime
- NVIDIA CUDA GPU. Current Nexus runtime setup is tested and supported only for NVIDIA CUDA graphics cards.
...
`archive` (or `zip`) creates the portable ZIP after packaging.<br>
Omit it for faster local test builds that only need the release folder.
...
The console's Nexus log action should open the latest Nexus app log in the default text app.<br>
...
Architecture, ownership, runtime layout, UI safety, and build checks are in the<br>
developer guide: [English](docs/DEVELOPERS.md) | [한국어](docs/DEVELOPERS.ko.md).
