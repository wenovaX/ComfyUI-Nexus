...
internal sealed class WorkflowTabController
{
	...
	private static readonly TimeSpan OptimisticActiveTimeout = TimeSpan.FromSeconds(2);
	...
	{
		if (index < 0 || _workflows.All(workflow => workflow.OriginalIndex != index))
		{
			return;
		}

		_optimisticActiveTabIndex = index;
		_optimisticActiveStartedUtc = DateTime.UtcNow;
		_activeTabIndex = index;
		_activeWorkflowState = _workflows.FirstOrDefault(workflow => workflow.OriginalIndex == index);
		RefreshFromLastSync();
	}

	private void ClearOptimisticActiveTab(int index)
	{
		if (_optimisticActiveTabIndex == index)
		{
			_optimisticActiveTabIndex = null;
		}

		RefreshFromLastSync();
	}

	private async Task SendTabActionAsync(string action, WorkflowTabInfo workflow)
	{
		NexusLog.Info($"Workflow action: {action} [{workflow.OriginalIndex}]");
		string payloadJson = JsonSerializer.Serialize(new
		{
			action,
			index = workflow.OriginalIndex,
			id = workflow.Id,
			path = workflow.Path,
			relativePath = workflow.RelativePath,
		});
		await SendNexusActionAsync(BridgeActions.TabCommandAction, payloadJson);
	}

	private async Task SendTabActionAsync(string action, int logicalIndex)
	{
		NexusLog.Info($"Workflow action: {action} [{logicalIndex}]");
		await SendNexusActionAsync(BridgeActions.TabCommandAction, $"{{ action: '{action}', index: {logicalIndex} }}");
	}

	private void RebuildTabs(JsonElement data)
	{
		using var operation = XamlUnhandledExceptionDiagnostics.EnterUiOperation("Header.Tabs.Rebuild");
		...
		foreach (var wf in data.EnumerateArray())
		{
			...
			string id = ReadWorkflowId(wf, index);
			...
			workflows.Add(new WorkflowTabInfo(index, id, name, displayName, active, modified, hasFile, path, relativePath));
			if (active)
			{
				payloadActiveIndex = index;
		...
		ApplyOptimisticActiveState(workflows, payloadActiveIndex, ref currentActiveIndex, ref activeName);
		...
		if (Application.Current?.Windows is { Count: > 0 })
		{
			...
			{
				window.Title = $"{SafeAppInfo.DisplayName} - [{activeName}]";
			}
		}
	...
	private void ApplyOptimisticActiveState(
		List<WorkflowTabInfo> workflows,
		int payloadActiveIndex,
		ref int currentActiveIndex,
		ref string activeName)
	{
		if (_optimisticActiveTabIndex is not int optimisticIndex)
		{
			return;
		}

		bool expired = DateTime.UtcNow - _optimisticActiveStartedUtc > OptimisticActiveTimeout;
		if (expired || optimisticIndex < 0 || optimisticIndex >= workflows.Count)
		{
			_optimisticActiveTabIndex = null;
			return;
		}

		if (payloadActiveIndex == optimisticIndex)
		{
			_optimisticActiveTabIndex = null;
			return;
		}
		...
		for (int i = 0; i < workflows.Count; i++)
		{
			workflows[i] = workflows[i] with { Active = i == optimisticIndex };
		}

		currentActiveIndex = optimisticIndex;
		activeName = workflows[optimisticIndex].DisplayName;
	}
	...
	private Border CreateOverflowButton()
	{
		...
		pointer.PointerEntered += (s, e) =>
		{
			...
			_ = SafeAnimation.ScaleToAsync(border, OverflowHoverScale, HoverAnimationLength, Easing.CubicOut, "WorkflowTabs.Overflow");
		};
		pointer.PointerExited += (s, e) =>
		{
			...
			_ = SafeAnimation.ScaleToAsync(border, 1, HoverAnimationLength, Easing.CubicIn, "WorkflowTabs.Overflow");
		};
	...
	private Border CreateStyledTab(string name, bool active, bool modified, bool hasFile, int index, double exactWidth)
	{
		...
		switchTap.Tapped += async (s, e) =>
		{
			...
			await ActivateWorkflowTabAsync(index);
		};
		...
		{
			using var operation = XamlUnhandledExceptionDiagnostics.EnterUiOperation("WorkflowTabs.CloseHoverIn");
			...
			var fadeOutDot = SafeAnimation.FadeToAsync(modifiedGlyphDot, 0, HoverAnimationLength, Easing.CubicOut, "WorkflowTabs.CloseHover");
			var shrinkDot = SafeAnimation.ScaleToAsync(modifiedGlyphDot, ModifiedGlyphHiddenScale, HoverAnimationLength, Easing.CubicOut, "WorkflowTabs.CloseHover");
			closeGlyphLabel.Scale = ModifiedGlyphHiddenScale;
			var fadeInX = SafeAnimation.FadeToAsync(closeGlyphLabel, 1, CloseGlyphRevealAnimationLength, Easing.CubicIn, "WorkflowTabs.CloseHover");
			var growX = SafeAnimation.ScaleToAsync(closeGlyphLabel, 1, CloseGlyphRevealAnimationLength, Easing.CubicOut, "WorkflowTabs.CloseHover");
			await Task.WhenAll(fadeOutDot, shrinkDot, fadeInX, growX);
		};
		closePointer.PointerExited += async (s, e) =>
		{
			using var operation = XamlUnhandledExceptionDiagnostics.EnterUiOperation("WorkflowTabs.CloseHoverOut");
			...
			var fadeOutX = SafeAnimation.FadeToAsync(closeGlyphLabel, 0, HoverAnimationLength, Easing.CubicOut, "WorkflowTabs.CloseHover");
			var shrinkX = SafeAnimation.ScaleToAsync(closeGlyphLabel, ModifiedGlyphHiddenScale, HoverAnimationLength, Easing.CubicOut, "WorkflowTabs.CloseHover");
			modifiedGlyphDot.Scale = ModifiedGlyphHiddenScale;
			var fadeInDot = SafeAnimation.FadeToAsync(modifiedGlyphDot, 1, CloseGlyphRevealAnimationLength, Easing.CubicIn, "WorkflowTabs.CloseHover");
			var growDot = SafeAnimation.ScaleToAsync(modifiedGlyphDot, 1, CloseGlyphRevealAnimationLength, Easing.CubicOut, "WorkflowTabs.CloseHover");
	...
	private static string ReadWorkflowId(JsonElement workflow, int fallbackIndex)
	{
		if (!workflow.TryGetProperty("id", out var idProperty))
		{
		...
		{
			JsonValueKind.String => idProperty.GetString() ?? fallbackIndex.ToString(CultureInfo.InvariantCulture),
	...
	private Border CreateDropdownItem(string name, bool active, bool modified, int index)
	{
		...
		pointer.PointerEntered += (s, e) =>
		{
			...
			_ = SafeAnimation.ScaleToAsync(border, DropdownHoverScale, HoverAnimationLength, Easing.CubicOut, "WorkflowTabs.Dropdown");
		};
		pointer.PointerExited += (s, e) =>
		{
			...
			_ = SafeAnimation.ScaleToAsync(border, 1, HoverAnimationLength, Easing.CubicIn, "WorkflowTabs.Dropdown");
		};
		border.GestureRecognizers.Add(pointer);
