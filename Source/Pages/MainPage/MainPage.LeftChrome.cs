...
public partial class MainPage
{
	...
	private static void TryEnsureDirectory(string path)
	{
		...
		{
			NexusLog.Warning($"Failed to ensure asset rail default directory '{path}': {ex.Message}");
		}
	}
	...
	private void ApplyRailWidth(double railWidth)
	{
		...
		RailControl.TranslationX = 0;
		RailResizeHandleControl.TranslationX = GetRailResizeHandleX(railWidth);
		SyncWebRailWidth(railWidth);
	}

	private void SyncWebRailWidth(double railWidth)
	{
		_ = _webViewBridge.SetRailWidthAsync(railWidth);
	}
}
