...
public partial class MainPage
{
	...
	private Task HandleUiStateUpdateAsync(JsonElement data)
	{
		...
		ApplyUiStateFlag(data, "linksHidden", ToolbarControl.SetLinksHidden);
		ApplyUiStateText(data, "zoomPercent", ToolbarControl.SetZoom);
		...
		ApplyUiStateFlag(data, "appModeOpen", ApplyAppModeSurfaceState);
		return Task.CompletedTask;
	}

	private void ApplyAppModeSurfaceState(bool isOpen)
	{
		HeaderControl.SetMainMenuAvailable(!isOpen);
		RailControl.SetAppModeSurfaceActive(isOpen);
	}
	...
	private static void ApplyUiStateText(JsonElement data, string propertyName, Action<string> apply)
	{
	...
	private Task HandleJsLogAsync(JsonElement data)
	{
		...
		{
			if (message.StartsWith("[APP_MODE]", StringComparison.Ordinal))
			{
				Log($"[JS] {message}");
			}
			else
	...
	private Task HandleBootReadyAsync(JsonElement data)
	{
		...
		SyncWebRailWidth(GetTargetRailWidth());
		_ = _webViewBridge.StartMediaAssetJobSyncAsync();
	...
	private void ExecuteControlDeckToggleBridgeDiagnostics()
	{
	...
	private async Task RunMediaAssetJobSnapshotBurstAsync(NexusOperationLease lease)
	{
		int previousOffsetMs = 0;
		foreach (int offsetMs in MediaAssetSnapshotBurstOffsetsMs)
		{
			int waitMs = offsetMs - previousOffsetMs;
			if (waitMs > 0 && !await lease.WaitForAsync(TimeSpan.FromMilliseconds(waitMs)))
			{
				return;
			}
