...
public partial class MainPage
{
	...
	private readonly StartupRouteDecider _startupRouteDecider;
	...
	private async Task StartStartupLoginSequenceSafeAsync()
	{
		...
		var routeReady = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
		Task splashBounces = PlayStartupSplashBouncesUntilReadyAsync(routeReady.Task);
	...
	private async Task LaunchNexusAppEntryAsync(CancellationToken cancellationToken)
	{
		...
		string comfyPath = _appManager.Paths.ActiveComfyPath;
	...
	private async Task ExecuteControlDeckPatchLocalHudAsync()
	{
		try
		{
			...
			var installService = _appManager.ComfyInstall;
	...
	private async Task ExecuteControlDeckPatchNexusBridgeAsync()
	{
		try
		{
			...
			string sourceBridgePath = Path.Combine(ComfyInstallService.RuntimePackagesPath, HudBridgeInstaller.NexusBridgePackageFolderName);
	...
	private async void OnSelectCoreClicked(object? sender, EventArgs e)
	{
		try
		{
			var result = await NexusAppManager.Instance.Platform.FilePicker.PickFolderAsync(LocalizationManager.Text("core_link.select_comfy_folder"));
	...
	{
		_appManager.Preferences.Set(PreferenceKeys.ComfyUIPath, path);
		SynchronizeConfiguredComfyPathSurfaces();
	...
	{
		var comfyRoot = _appManager.Paths.ConfiguredComfyPath;
	...
	private async Task ProbeComfyUIWorkflowPath()
	{
		...
		try
		{
			...
			var response = await httpClient.GetAsync(ComfyApiOptions.GetWorkflowProbeUrl(_appManager.Settings.Settings));
