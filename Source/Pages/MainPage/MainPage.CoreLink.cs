...
public partial class MainPage
{
	...
	private async void OnPatchLocalHudClicked(object? sender, EventArgs e)
	{
		try
		{
			...
			{
				Log("[HUD DEV] Local ComfyUI-HUD project was not found next to the Nexus workspace.");
			...
			Log($"[HUD DEV] Patching local ComfyUI-HUD from {localHudPath}");
		...
		{
			Log($"[HUD DEV] Patch failed: {ex.GetType().Name} - {ex.Message}");
			await DisplayAlertAsync(
				LocalizationManager.Text("core_link.hud_patch_failed_title"),
				ex.Message,
				LocalizationManager.Text("common.ok"));
	...
	private async void OnPatchNexusBridgeClicked(object? sender, EventArgs e)
	{
		try
		{
			string targetHudPath = Path.Combine(GetConfiguredCoreLinkPath(), "custom_nodes", "ComfyUI-HUD");
			...
			{
				Log($"[HUD DEV] Active ComfyUI-HUD install was not found: {targetHudPath}");
				await DisplayAlertAsync(
					LocalizationManager.Text("core_link.bridge_patch_unavailable_title"),
					LocalizationManager.Format("core_link.bridge_patch_missing_hud_message", targetHudPath),
					LocalizationManager.Text("common.ok"));
				return;
			}

			if (!Directory.Exists(sourceBridgePath))
			{
				Log($"[HUD DEV] Packaged Nexus bridge payload was not found: {sourceBridgePath}");
				await DisplayAlertAsync(
					LocalizationManager.Text("core_link.bridge_patch_unavailable_title"),
			...
			bool confirm = await DisplayAlertAsync(
				...
				LocalizationManager.Format("core_link.patch_nexus_bridge_message", sourceBridgePath, Path.Combine(targetHudPath, "js")),
				LocalizationManager.Text("core_link.patch"),
				LocalizationManager.Text("common.cancel"));
			if (!confirm) return;
			...
			var installService = ComfyInstallService.Instance ?? new ComfyInstallService();
			installService.OnMessage = message => Log(message);
			Log($"[HUD DEV] Patching Nexus bridge into {targetHudPath}");
		...
		{
			Log($"[HUD DEV] Nexus bridge patch failed: {ex.GetType().Name} - {ex.Message}");
			await DisplayAlertAsync(
				LocalizationManager.Text("core_link.bridge_patch_failed_title"),
				ex.Message,
				LocalizationManager.Text("common.ok"));
	...
	private static IEnumerable<string> EnumerateLocalHudProjectCandidates()
	{
		...
		foreach (string root in EnumeratePossibleNexusRoots())
		{
			...
			{
				string siblingHud = Path.Combine(parent, "ComfyUI-HUD");
