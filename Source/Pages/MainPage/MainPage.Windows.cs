...
public partial class MainPage
{
	...
	{
		WorkspaceControl.BrowserView.HandlerChanged += async (s, e) => {
		...
		this.HandlerChanged += (s, e) => {
			if (this.Handler?.PlatformView is Microsoft.UI.Xaml.FrameworkElement)
			{
				...
				if (window?.Content is Microsoft.UI.Xaml.FrameworkElement windowContent)
				{
					...
					windowContent.AddHandler(
						...
						new PointerEventHandler((sender, args) => {
		...
		RailResizeHandleControl.HandleElement.HandlerChanged += (s, e) => ConfigureRailResizeCursor();
	}
