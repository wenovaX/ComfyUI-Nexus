...
public partial class MainPage
{
	...
	private const string WorkflowIndexOperationKey = "workflow-index";
	...
	private void SynchronizeConfiguredComfyPathSurfaces()
	{
		...
		// A freshly installed runtime does not have a valid ComfyUI root during the
		// initial shell construction. Re-apply the default Output root once setup
		// has established the configured path.
		InitializeRailRoot();
	...
	private Task RefreshWorkflowIndexAndWebAsync()
		=> RunWorkflowIndexRefreshAsync(
			new NexusOperationLease(
	...
	private async Task RunWorkflowIndexRefreshAsync(NexusOperationLease lease, bool waitForQuietPeriod)
	{
		if (waitForQuietPeriod && !await lease.WaitForAsync(TimeSpan.FromMilliseconds(WorkflowIndexWatcherDebounceMs)))
		{
			return;
		}
		...
		await _workflowIndexRefreshGate.WaitAsync(lease.LifecycleToken);
		try
		{
			if (!lease.IsCurrent)
			{
			...
			await RefreshWorkflowIndexAsync(lease);
			if (!lease.IsCurrent)
			{
				return;
			}
	...
	private async Task RefreshWorkflowIndexAsync(NexusOperationLease lease)
	{
		await _workflowIndexScanGate.WaitAsync(lease.LifecycleToken);
		try
		{
			if (!lease.IsCurrent || string.IsNullOrWhiteSpace(_comfyWorkflowsPath) || !Directory.Exists(_comfyWorkflowsPath))
			{
				return;
			}
			...
			var relativePaths = Directory.EnumerateFiles(_comfyWorkflowsPath, "*.json", enumerationOptions)
				...
				.ToArray();
			HashSet<string> knownWorkflowDirectories = Directory.EnumerateDirectories(_comfyWorkflowsPath, "*", enumerationOptions)
			...
			await MainThread.InvokeOnMainThreadAsync(async () =>
			{
				...
				_knownWorkflowDirectories = knownWorkflowDirectories;
	...
	private async Task CompleteWorkflowAssetMutationAsync(AssetPathMutation mutation, bool succeeded)
	{
		...
		try
		{
			...
			// The filesystem mutation and local workflow state are committed above. Web app-data
			// refresh is post-commit synchronization and must not hold the dialog transaction open.
			QueueCommittedWorkflowIndexRefresh();
	...
	private async Task HandleWorkflowTabActionAsync(WorkflowTabController.WorkflowTabInfo workflow, WorkflowActionKind kind)
	{
		...
		if (kind == WorkflowActionKind.Rename)
		{
			...
			await RenameTrackedWorkflowAsync(workflow);
			return;
		}

		string? action = GetWorkflowCommandAction(kind);
		if (!string.IsNullOrWhiteSpace(action))
		{
			await InvokeWorkflowTabCommandActionAsync(workflow, action);
		}
	}

	private static string? GetWorkflowCommandAction(WorkflowActionKind kind)
	{
		return kind switch
		{
	...
	private Task InvokeWorkflowTabCommandActionAsync(WorkflowTabController.WorkflowTabInfo workflow, string workflowAction)
	{
		...
		return _webViewBridge.InvokeActionAsync(BridgeActions.TabCommandAction, payloadJson);
	}

	private async Task RenameTrackedWorkflowAsync(WorkflowTabController.WorkflowTabInfo workflow)
	{
