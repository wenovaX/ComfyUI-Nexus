...
internal sealed class ManagerExtensionDiagnosticNode : IConfigurableDiagnosticNode
{
	...
	public Task<HealthState> CheckHealthAsync(CancellationToken cancellationToken)
	{
		try
		{
			string customNodesPath = Path.Combine(ComfyInstallService.ComfyPath, "custom_nodes");
			string managerPath = Path.Combine(customNodesPath, "ComfyUI-Manager");
			string hudPath = Path.Combine(customNodesPath, "ComfyUI-HUD");
			...
			foreach (var node in essentialNodes)
			{
				...
				if (node.Folder == "ComfyUI-Manager" || node.Folder == "ComfyUI-HUD") continue;
		...
		catch (Exception ex)
		{
			...
			EnvironmentPath = Path.Combine(ComfyInstallService.ComfyPath, "custom_nodes");
