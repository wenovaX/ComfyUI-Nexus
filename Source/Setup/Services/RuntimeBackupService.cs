...
internal sealed class RuntimeBackupService
{
	...
	private static IReadOnlyList<string> NormalizeTargets(IEnumerable<string> targets)
		=> targets
			.Where(target => target is RuntimeBackupTargets.Models or RuntimeBackupTargets.CustomNodes)
			.Distinct(StringComparer.OrdinalIgnoreCase)
			.ToList();
	...
	private static string NormalizeRelativePath(string path)
	{
		...
		if (string.Equals(target, RuntimeBackupTargets.Models, StringComparison.OrdinalIgnoreCase))
		{
			target = RuntimeBackupTargets.Models;
		}
		else if (string.Equals(target, RuntimeBackupTargets.CustomNodes, StringComparison.OrdinalIgnoreCase))
		{
			target = RuntimeBackupTargets.CustomNodes;
		}
