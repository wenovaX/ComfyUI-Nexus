...
internal static class ExtraModelPathsService
{
	...
	private static readonly string[] RequiredModelSubdirectories =
	[
		"checkpoints",
		"loras",
		"vae",
	...
	internal static ExtraModelPathsResult Inspect(SetupSettings settings, string comfyPath)
	{
		...
		IReadOnlyList<ModelPathMapping> mappings = ResolveModelPathMappings(comfyPath);
		string expected = BuildManagedSection(roots, mappings);
		...
		out ExtraModelPathsTransaction? transaction)
	{
		...
		try
		{
			...
			IReadOnlyList<string> roots = NormalizeRoots(settings);
			IReadOnlyList<ModelPathMapping> mappings = ResolveModelPathMappings(comfyPath);
			IReadOnlyList<string> subdirectories = ResolveModelDirectoryStructure(comfyPath, mappings);
			...
			string finalText = BuildFinalText(unmanagedText, roots, mappings, newline);
	...
	private static string BuildFinalText(
		...
		IReadOnlyList<ModelPathMapping> mappings,
		string newline)
	{
		if (roots.Count == 0)
		{
		...
		return unmanagedText
			+ separator
			+ BuildManagedSection(roots, mappings).Replace(Environment.NewLine, newline, StringComparison.Ordinal)
			+ newline;
	}

	private static string BuildManagedSection(
		IReadOnlyList<string> roots,
		IReadOnlyList<ModelPathMapping> mappings)
	{
		...
		for (int index = 0; index < roots.Count; index++)
		{
			...
			builder.AppendLine($"  base_path: {QuoteYamlScalar(roots[index].Replace('\\', '/'))}");

			foreach (ModelPathMapping mapping in mappings)
			{
	...
	private static IReadOnlyList<ModelPathMapping> ResolveModelPathMappings(string comfyPath)
	{
		var subdirectories = new HashSet<string>(
			RequiredModelSubdirectories,
			StringComparer.OrdinalIgnoreCase);
		string modelsPath = Path.Combine(comfyPath, "models");
		if (Directory.Exists(modelsPath))
		{
			foreach (string directory in Directory.EnumerateDirectories(modelsPath, "*", SearchOption.TopDirectoryOnly))
			{
		...
		foreach ((string key, string[] paths) in CorePathAliases)
		{
			List<string> availablePaths = paths
				.Where(path => PathExistsInModelStructure(modelsPath, subdirectories, path))
				.ToList();
	...
	private static IReadOnlyList<string> ResolveModelDirectoryStructure(
		string comfyPath,
		IReadOnlyList<ModelPathMapping> mappings)
	{
		var relativePaths = new HashSet<string>(
			mappings.SelectMany(mapping => mapping.RelativePaths),
			StringComparer.OrdinalIgnoreCase);
		string modelsPath = Path.Combine(comfyPath, "models");
		if (!Directory.Exists(modelsPath))
		{
		...
		foreach (string directory in EnumerateDirectoriesWithoutReparsePoints(modelsPath))
		{
			string relativePath = Path.GetRelativePath(modelsPath, directory)
				.Replace('\\', '/');
	...
	private static IEnumerable<string> EnumerateDirectoriesWithoutReparsePoints(string rootPath)
	{
