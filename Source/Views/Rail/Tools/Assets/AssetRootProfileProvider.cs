...
internal static class AssetRootProfileProvider
{
	...
	internal static AssetRootProfile ResolveForPath(string? path, string comfyRoot, string fixedWorkflowsPath)
	{
		...
		var profiles = GetFixedProfiles(comfyRoot, fixedWorkflowsPath);
		if (string.IsNullOrWhiteSpace(normalizedPath))
		{
			return profiles.FirstOrDefault(profile => string.Equals(profile.Id, "output", StringComparison.OrdinalIgnoreCase))
	...
	private static void TryEnsureDirectory(string path)
	{
		...
		{
			// The asset rail can still show other roots if the output folder cannot be created.
		}
	}
}
