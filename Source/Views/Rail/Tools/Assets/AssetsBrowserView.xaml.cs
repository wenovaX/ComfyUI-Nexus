...
public partial class AssetsBrowserView : ContentView, IAssetRailTool
{
	...
	private static readonly Color BookmarkActionColor = Color.FromArgb("#8de7ff");
	...
	private static readonly Color AssetDropHighlightColor = Color.FromArgb("#8de7ff");
	...
	private readonly AssetTreeWatcherController _treeWatcher;
	...
	private CancellationTokenSource? _modelThumbnailHoverCancellation;
	...
	public AssetsBrowserView()
	{
		...
		new NexusEntryTextController(RailSearchEntry, RailSearchBorder);
		...
		_treeWatcher = new AssetTreeWatcherController(
			() => _rootPath,
			() => CurrentWatcherOptions,
			() => _isAnimating || _isTreeLoading || _isApplyingWatcherChanges,
	...
	{
		// Startup prewarm is intentionally limited to reusable rows and lightweight data.
		// Visible materialization belongs to OpenAsync after the rail animation is done.
		...
		if (_rootNodes.Count == 0 || _treeWatcher.IsDirty)
		{
	...
	async Task IRailToolView.OpenAsync(CancellationToken cancellationToken)
	{
		...
		if (_rootNodes.Count == 0 || _treeWatcher.IsDirty)
		{
			RailPerformanceDiagnostics.Mark("AssetsOpenBuildTreeStart", perf, $"nodes={_rootNodes.Count}, dirty={_treeWatcher.IsDirty}");
			await BuildTreeAsync();
	...
	private async Task BuildTreeAsync(bool clearDataCache = false)
	{
		...
		try
		{
			...
			_isTreeLoading = true;
			_treeWatcher.ResetDirtyTracking();
	...
	private View CreateWorkflowBookmarkRow(WorkflowBookmarkAssetItem item)
	{
		...
		{
			Text = "{}",
			TextColor = Color.FromArgb("#31d8ff"),
			FontSize = 10,
	...
	{
		_treeWatcher.ResetDirtyTracking();
		MainThread.BeginInvokeOnMainThread(() => ReloadTree(resetExpansion: false, invalidateLayout: true));
	}
	...
	private void BeginModelThumbnailHover(string fullPath, string displayName, bool isDirectory, VisualElement anchor)
	{
		...
		_modelThumbnailHoverCancellation?.Cancel();
		_modelThumbnailHoverCancellation?.Dispose();
		var cts = new CancellationTokenSource();
		_modelThumbnailHoverCancellation = cts;

		_ = ShowModelThumbnailPreviewAfterDelayAsync(fullPath, displayName, anchor, version, cts.Token);
	}
	...
	private async Task ShowModelThumbnailPreviewAfterDelayAsync(
		string fullPath,
		string displayName,
		...
		CancellationToken cancellationToken)
	{
		try
		{
			await Task.Delay(ModelThumbnailHoverDelayMs, cancellationToken);
			...
			{
				if (version != _modelThumbnailHoverVersion || cancellationToken.IsCancellationRequested)
				{
					return;
				}
		...
		{
			NexusLog.Trace($"[ASSET_TREE] Thumbnail preview failed for '{fullPath}': {ex.Message}");
		}
	}
	...
	private void HideModelThumbnailPreview(bool clearCache = false)
	{
		_modelThumbnailHoverCancellation?.Cancel();
		_modelThumbnailHoverCancellation?.Dispose();
		_modelThumbnailHoverCancellation = null;
