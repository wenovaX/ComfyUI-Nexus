...
public partial class AssetsBrowserView
{
	...
	private async Task RefreshSearchResultsAsync(bool immediate = false)
	{
		...
		try
		{
			if (!immediate)
			{
				await Task.Delay(AssetBrowserOptions.SearchDebounceDelay);
			...
			var results = await _assetHubService.SearchAsync(
				_rootPath,
				query,
				recursive: CurrentUsesRecursiveSearch,
				filterModelFilesOnly: CurrentFiltersModelFilesOnly,
				includeDirectories: CurrentSearchIncludesDirectories,
			...
			MainThread.BeginInvokeOnMainThread(() => ApplySearchResults(query, results));
		}
