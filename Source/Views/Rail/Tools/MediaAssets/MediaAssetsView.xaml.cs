...
public partial class MediaAssetsView : ContentView, IRailToolView
{
	...
	private static readonly Color MediaToolbarHoverShadowColor = Color.FromArgb("#2a7598");
	...
	private IPlatformDirectoryWatcherSubscription? _watcher;
	...
	private int _watcherRefreshVersion;
	...
	public MediaAssetsView()
	{
		...
		new NexusEntryTextController(MediaAssetsSearchEntry, MediaAssetsSearchBorder);
	...
	async Task IRailToolView.PrewarmAsync(CancellationToken cancellationToken)
	{
		...
		RestartWatcher();
		await GetSurface(MediaAssetScope.Output).CellPool!.PrewarmAsync(MediaCellPrewarmCount, MediaCachedThumbnailBatchSize);
		await Task.Yield();
		await GetSurface(MediaAssetScope.Input).CellPool!.PrewarmAsync(MediaCellPrewarmCount, MediaCachedThumbnailBatchSize);
	}
	...
	async Task IRailToolView.OpenAsync(CancellationToken cancellationToken)
	{
		...
		await RefreshAsync(cancellationToken, showLoadingOverlay: false);
	...
	{
		Interlocked.Increment(ref _watcherRefreshVersion);
		DisposeWatcher();
	}
	...
	private void RestartWatcher()
	{
		...
		string rootPath = GetRootPath(MediaAssetScope.Input);
		if (string.IsNullOrWhiteSpace(rootPath) || !Directory.Exists(rootPath))
		{
			return;
		}
		...
		var watchResult = PlatformManager.Current.DirectoryWatcher.TryWatch(
			...
			_ => QueueWatcherRefresh(),
			exception =>
			{
				if (exception != null)
				{
					NexusLog.Warning($"Media assets watcher failed: {exception.Message}");
				}
				QueueWatcherRefresh();
			});
		...
		if (!watchResult.IsSuccess || watchResult.Value is null)
		{
			...
			{
				NexusLog.Warning($"Media assets watcher unavailable: {watchResult.Message}");
			}
			return;
		}

		_watcher = watchResult.Value;
		_watcher.Start();
	}
	...
	private void QueueWatcherRefresh()
	{
		...
		int refreshVersion = Interlocked.Increment(ref _watcherRefreshVersion);
		...
		_ = Task.Run(async () =>
		{
			try
			{
				...
				if (refreshVersion == _watcherRefreshVersion)
				{
					await MainThread.InvokeOnMainThreadAsync(() =>
						_ = RefreshScopeAsync(
							MediaAssetScope.Input,
							CancellationToken.None,
							showLoadingOverlay: _activeScope == MediaAssetScope.Input,
							forceRefresh: true));
				}
			}
			catch (Exception ex) when (ex is not OperationCanceledException)
			{
				NexusLog.Warning($"Media assets watcher refresh failed: {ex.Message}");
			}
		});
	}
	...
	{
		var surface = GetSurfaceContaining(entry.FullPath) ?? GetActiveSurface();
		var items = surface.Items
			.Where(item => File.Exists(item.FullPath))
			.Select(ToMediaViewerItem)
			.ToList();
		int startIndex = items.FindIndex(item => string.Equals(item.FullPath, entry.FullPath, StringComparison.OrdinalIgnoreCase));
		if (items.Count == 0 || startIndex < 0 || ViewerRequested == null)
		{
			return false;
		}

		ViewerRequested.Invoke(this, new MediaAssetViewerRequest(items, startIndex));
		return true;
	}
	...
	private static void OnToolbarButtonPointerEntered(object? sender, PointerEventArgs e)
	{
		if (sender is Border border)
		{
			...
			{
				Brush = MediaToolbarHoverShadowColor,
