...
{
	private const string NodeDefaultAccentColor = "#8de7ff";
	...
	public string HeaderAccentColor => Section switch
	{
		SectionKind.NexusFamily => "#00d2ff",
		SectionKind.Bookmarked => "#ffd700",
		SectionKind.Blueprints => "#da70d6",
		SectionKind.Partner => "#7be495",
		SectionKind.Comfy => "#8dbce0",
		SectionKind.Extensions => "#ff8c5a",
		_ => "#5c7a99"
	};
	...
	public string ColorHex => NodeEntry?.ColorHex ?? NodeDefaultAccentColor;
	...
	public string BookmarkToggleColor => IsBookmarked ? "#ffd700" : "#6d8399";
...
public partial class NodeLibraryView : ContentView, IRailToolView
{
	...
	async Task IRailToolView.OpenAsync(CancellationToken cancellationToken)
	{
		...
		if (FlattenedNodes.Count == 0)
		{
			...
			await RefreshTreeAsync(forceRebuild: false, showLoading: false);
