...
public sealed class VirtualizedVerticalListView : ContentView
{
	...
	private CancellationTokenSource _attachCts = new();
	...
	{
		int updateVersion = BeginVisibleUpdate(out var attachToken);
		using var linkedCts = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken, attachToken);
		...
		{
			if (effectiveToken.IsCancellationRequested || updateVersion != _visibleUpdateVersion)
			{
				return;
			}
	...
	private int BeginVisibleUpdate(out CancellationToken cancellationToken)
	{
		_attachCts.Cancel();
		_attachCts.Dispose();
		_attachCts = new CancellationTokenSource();
		cancellationToken = _attachCts.Token;
		...
		CancellationToken cancellationToken)
	{
		...
		{
			await Task.Delay(coalesceDelayMs, cancellationToken);
		...
		foreach (int index in indexes)
		{
			if (cancellationToken.IsCancellationRequested || updateVersion != _visibleUpdateVersion || index < 0 || index >= _items.Count)
			{
				return;
			}
