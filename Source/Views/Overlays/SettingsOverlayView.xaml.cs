...
public partial class SettingsOverlayView : ContentView, INexusPopupSurface
{
	...
	{
		_gpuDevices.Clear();
		_gpuDevices.AddRange(GpuDiscoveryService.GetCachedDevicesOrFallback());
		RebuildGpuOptions();
		UpdateGpuSelectionVisuals();
	}
	...
	private static string GetEffectiveComfyPath(SetupSettings settings)
	{
		...
		return string.IsNullOrWhiteSpace(activePath) ? ComfyPathResolver.ResolveManagedComfyPath() : activePath;
	}

	private static void SyncActiveComfyPathPreference(SetupSettings settings)
	{
		string comfyPath = string.Equals(settings.InstallMode, SetupInstallModes.ExistingComfyPath, StringComparison.Ordinal)
			? settings.ComfyPath
			: ComfyPathResolver.ResolveManagedComfyPath();
	...
	private void SelectComfyCoreSource(string source)
	{
		...
		if (string.Equals(source, SetupSettingsService.Instance.Settings.ComfyCoreSource, StringComparison.Ordinal))
		{
