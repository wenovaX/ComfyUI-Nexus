...
{
	private const double DialogShownGlowOpacity = 0.18;
	...
	private const uint ShowGlowLength = 220;
	private const string DialogGlowAnimationName = "DialogPanelGlow";
	...
	public NexusDialogOverlayView()
	{
		...
		new NexusEntryTextController(DialogPromptEntry, DialogPromptFrame);
	...
	private async Task DrainQueueAsync()
	{
		...
		while (_queue.Count > 0)
		{
			...
			finally
			{
				...
				this.AbortAnimation(DialogGlowAnimationName);
				Opacity = 0;
				DialogPanelGlow.Opacity = 0;
				IsVisible = false;
				_completion = null;
	...
	private ThumbnailChoiceCardState CreateThumbnailChoiceCard(NexusDialogThumbnailChoice choice)
	{
		...
		imageFrame.Shadow = new Shadow
		{
			Brush = isSelected ? Color.FromArgb("#31D8FF") : Color.FromArgb("#5B86A6"),
			Radius = isSelected ? 18 : 10,
			Offset = new Point(0, 8),
			Opacity = isSelected ? 0.32f : 0.16f
		};
		...
		{
			Brush = isSelected ? Color.FromArgb("#22E8FF") : Color.FromArgb("#0E5A78"),
			Radius = isSelected ? 22 : 14,
			Offset = new Point(0, 8),
			Opacity = isSelected ? 0.28f : 0.14f
		};
	...
	private Task AnimateShowAsync()
		=> Task.WhenAll(
			this.FadeToAsync(1, ShowFadeLength, Easing.CubicOut),
			AnimateDialogGlowAsync(DialogShownGlowOpacity, ShowGlowLength, Easing.CubicOut));

	private Task AnimateDialogGlowAsync(double targetOpacity, uint length, Easing easing)
	{
		var completion = new TaskCompletionSource();
		new Animation(value => DialogPanelGlow.Opacity = (float)value, DialogPanelGlow.Opacity, targetOpacity, easing)
			.Commit(this, DialogGlowAnimationName, 16, length, finished: (_, _) => completion.TrySetResult());
	...
	private void OnDialogPromptEntryFocused(object? sender, FocusEventArgs e) => ApplyPromptFocusState(true);
	...
	private void ApplyPromptFocusState(bool isFocused)
	{
		...
		DialogPromptGlow.Opacity = isFocused ? 0.22f : 0.08f;
	}
