...
public partial class HelpOverlayView : ContentView
{
	...
	private const string PanelGlowAnimationName = "HelpPanelGlow";
	...
	internal async Task PrewarmLayoutAsync()
	{
		...
		float previousGlowOpacity = HelpPanelGlow.Opacity;
		...
		finally
		{
			...
			HelpPanelGlow.Opacity = previousGlowOpacity;
		}
	}
	...
	private Task AnimatePanelGlowAsync(double targetOpacity, uint length, Easing easing)
	{
		...
		new Animation(value => HelpPanelGlow.Opacity = (float)value, HelpPanelGlow.Opacity, targetOpacity, easing)
			.Commit(this, PanelGlowAnimationName, 16, length, finished: (_, _) => completion.TrySetResult());
	...
	private void SelectEntry(HelpNavigationEntry entry)
	{
		...
		if (ReloadContentOnTabChange && !_isRenderingContent && _entries.Count > 0)
		{
			...
			RenderContent(selectedIndex < 0 ? 0 : selectedIndex);
			return;
		}
	...
	private void ApplyDetailAccent(string accentColor)
	{
		...
		HelpDetailAccentShadow.Brush = color;
	}
