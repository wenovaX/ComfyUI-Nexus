...
- 예측 가능한 상태 동기화를 위한 file watcher, 명시적 bridge action, bridge 자동 복구,
  diagnostic log.
- watcher, media, GPU refresh 작업은 latest-wins 방식으로 조정해 stale background 결과가
  현재 desktop state를 덮어쓰지 않게 합니다.
...
`[UI_TRACE]`는 최근의 주요 surface 및 lifecycle 전이를 기록하며, native 종료 뒤에도 이전 session marker에 이 trace가 보존됩니다.

Nexus 로그에 managed exception이 남지 않고 앱이 종료되었다면
