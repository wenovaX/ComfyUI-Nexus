...
if "%PACKAGE_MODE%"=="" (
    ...
    echo.
    echo Usage:
    echo   dev-build-as-binary.bat [configuration] [folder^|single] [clean] [archive^|zip] [--cert name]
    echo.
    echo Examples:
...
if not "%CERT_NAME%"=="" (
    ...
    powershell -NoProfile -ExecutionPolicy Bypass -File "%CD%\dev-check-code-signing-certificate.ps1" -Name "%CERT_NAME%"
    if errorlevel 1 (
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%CD%\dev-build-portable-release.ps1" ^
