...
$portableReadmes = @(
	@{ Source = Join-Path $ProjectRoot "Resources\Distribution\PortableReadme.txt"; RelativePath = "README.txt" },
	@{ Source = Join-Path $ProjectRoot "Resources\Distribution\PortableReadme.ko-KR.txt"; RelativePath = "README.ko-KR.txt" },
	@{ Source = Join-Path $ProjectRoot "Resources\Distribution\PortableReadme.zh-Hans.txt"; RelativePath = "README.zh-Hans.txt" },
	@{ Source = Join-Path $ProjectRoot "Resources\Distribution\PortableReadme.zh-Hant.txt"; RelativePath = "README.zh-Hant.txt" }
)
...
Write-Host "[Nexus] Adding portable release guides..."
foreach ($readme in $portableReadmes) {
	Copy-ReleaseFile -Source $readme.Source -RelativePath $readme.RelativePath
}
...
foreach ($readme in $portableReadmes) {
	Test-RequiredReleaseFile -RelativePath $readme.RelativePath
}
if ($PackageMode -eq "folder") {
