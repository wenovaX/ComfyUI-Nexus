import { importFirstCached, registerComfyInternalHelper } from "./comfy_internal/registry.js";

const MANAGER_MODULES = {
	common: [
		"/extensions/ComfyUI-Manager/common.js",
		"/extensions/ComfyUI-Manager/js/common.js",
	],
	customNodes: [
		"/extensions/ComfyUI-Manager/custom-nodes-manager.js",
		"/extensions/ComfyUI-Manager/js/custom-nodes-manager.js",
	],
};
...
async function getManagerCommonModule() {
	return importFirstCached("ComfyUIManager.common", MANAGER_MODULES.common);
}

export function getComfyManagerMemoryHelper(bridge = null) {
	return registerComfyInternalHelper("ComfyManagerMemory", () => ({
		getCommon: getManagerCommonModule,
		async unloadModels() {
			const { free_models } = await getManagerCommonModule();
			if (typeof free_models !== "function") {
				throw new Error("ComfyUI-Manager free_models export not found");
			}

			await free_models(false);
			return true;
		},
		async freeModelAndNodeCache() {
			const { free_models } = await getManagerCommonModule();
			if (typeof free_models !== "function") {
				throw new Error("ComfyUI-Manager free_models export not found");
			}
...
async function importManagerModules() {
	const [common, customNodes] = await Promise.all([
		getManagerCommonModule(),
		importFirstCached("ComfyUIManager.customNodes", MANAGER_MODULES.customNodes),
	]);
