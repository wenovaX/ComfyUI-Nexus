...
export function setupModeTracker(bridge) {
    ...
    const checkMode = () => {
        ...
        const btn = document.querySelector('button[aria-label="Canvas Mode"]');
        if (!btn) return;

        const isHand = !!btn.querySelector('.icon-\\[lucide--hand\\]');
        const isSelect = !!btn.querySelector('.icon-\\[lucide--mouse-pointer-2\\]');

        let mode = "UNKNOWN";
        if (isHand) mode = "HAND";
        else if (isSelect) mode = "SELECT";
